...
app.use(helmet({
  ...
  referrerPolicy: { policy: "strict-origin-when-cross-origin" } // Fix: Allows our own frontend to send referer
}));
...
app.get('/proxy', botBlocker, requireApiKey, async (req, res) => {
  ...
  try{
    ...
    // We now use native fetch() for everything since it is no longer blocked and doesn't corrupt binary on Windows
    ...
    if (isPlaylistOrSub) {
      ...
      const baseUrl = new URL(fetchRes.url); // Use the final URL after any redirects!
      ...
      const rewritten = text.replace(/https?:\/\/[^\s\'\"]+/g, m => {
        ...
        if (req.query.apiKey) proxyUrl += '&apiKey=' + encodeURIComponent(req.query.apiKey);
        return proxyUrl;
      });
      res.send(rewritten);
    } else {
      // For MP4 or TS segments, pipe the binary stream directly!
      // This guarantees no Windows \n to \r\n corruption
      ...
      req.on('close', () => {
         // If the user closes the video player tab, destroy the stream to save bandwidth
