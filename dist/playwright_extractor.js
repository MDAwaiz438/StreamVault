...
function decode1EmbedProxyUrl(url) {
    try {
        ...
        if (b64) {
            ...
            if (httpIdx !== -1)
                decoded = decoded.substring(httpIdx);
            return decoded;
        }
    }
    catch (e) { }
...
async function captureMediaFromUrl(targetUrl, outFile = 'captures.json') {
    let browser = null;
    ...
    page.on('response', async (response) => {
        try {
            ...
            if (seen.has(url) && captures.some(c => c.url === url && c.type === 'response'))
                return;
