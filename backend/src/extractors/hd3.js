...
export default {
  extract: async ({ tmdbId, type, s, e, req, res, cache, cacheKey, encryptURL, fallback, server }) => {
    // vixsrc.to places its entire domain (including /playlist endpoints) behind 
    // a strict Cloudflare Turnstile that actively blocks headless browsers (Playwright) 
    // and custom TLS clients (curl_cffi). 
    // However, the underlying media is identical to the reliable vidnest CDN.
    // To ensure 100% uptime for the vixsrc server ID without human-captcha solving,
    // we route the extraction through the unblocked vidnest API using curl_cffi.
