...
const topEnd = '  const [progress, setProgress] = useState(0);';
const bottomStart = '                    else videoRef.current.requestPictureInPicture();';

const topMatch = content.indexOf(topEnd);
const top = content.substring(0, topMatch + topEnd.length);

const bottomMatch = content.indexOf(bottomStart);
const bottom = content.substring(bottomMatch);
        ...
        >
          ...
          <div className={\bsolute bottom-0 left-0 w-full flex flex-col transition-opacity duration-300
            ...
            <div className="relative h-[3px] bg-white/20 w-full cursor-pointer group/scrubber hover:h-[5px] transition-all" onClick={handleSeek}>
              <div className="absolute top-0 left-0 h-full bg-white/40 transition-all duration-200" style={{ width: \\%\ }}></div>
              <div className="absolute top-0 left-0 h-full bg-[#00a8e8]" style={{ width: \\%\ }}>
                    ...
                    {settingsMenuState === 'main' && (
                      <div className="py-2">
                        <button onClick={(e) => { e.stopPropagation(); setSettingsMenuState('quality'); }} className="w-full px-4 py-2 text-[13px] font-medium flex items-center justify-between hover:bg-white/10 transition-colors">
                          ...
                          <span className="flex items-center gap-1 text-white/50 text-[11px]">{currentLevel === -1 ? 'Auto' : (levels[currentLevel]?.height ? \\p\ : '')} <ChevronRight className="w-3 h-3" /></span>
                        </button>
...
fs.writeFileSync('app/page.js', top + middle + bottom);
