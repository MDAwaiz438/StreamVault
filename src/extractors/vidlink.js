        ...
        try {
          ...
          if (Array.isArray(data)) {
            ...
            if (mp4Res) {
              ...
              return res.json({ encryptedStream: encryptURL(mp4Res.url), server });
            }
            ...
            if (mp4Req) {
              ...
              return res.json({ encryptedStream: encryptURL(mp4Req.url), server });
            }
            ...
            if (m3u8Req) {
              ...
              return res.json({ encryptedStream: encryptURL(m3u8Req.url), server });
            }
          }
        } catch(e) { console.error('vidlink parse error', e); }
      }
      return res.status(404).json({error: 'Stream URL not found on vidlink.pro'});
    });
  }
};
